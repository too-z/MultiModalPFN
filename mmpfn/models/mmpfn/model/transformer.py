...
from mmpfn.models.mmpfn.model.encoders import (
    LinearInputEncoderStep,
    NanHandlingEncoderStep,
    SequentialEncoder,
)
from mmpfn.models.mmpfn.model.layer import PerFeatureEncoderLayer

DEFAULT_EMSIZE = 128



class MultiheadGatedMLP(nn.Module):
    def __init__(self, in_dim=768, out_dim=192, mgm_heads=8, dropout=0.1):
        super().__init__()
    ...
    def orthogonality_loss(self):
        ...
        for i in range(len(self.projs)):
            for j in range(i+1, len(self.projs)):
                W_i = self.projs[i][-1].weight
                W_j = self.projs[j][-1].weight
                loss += torch.norm(W_i @ W_j.T, p="fro") # Frobenius norm of cross-correlation
        return loss
...
class CrossQueryAttentionMerger(nn.Module):
    ...
    def forward(self, src):
        ...
        out = out.transpose(0, 1) # (batch, sequence, embed)
...
class PerFeatureTransformer(nn.Module):
    ...
    def forward(self, *args: Any, **kwargs: Any) -> dict[str, torch.Tensor]:  # noqa: D417
        ...
        if args == () and all(k in kwargs for k in ("train_x", "train_y", "test_x", "train_image", "test_image")):
            # for finetuning
            assert "single_eval_pos" not in kwargs
            x = kwargs.pop("train_x")
            image = kwargs.pop("train_image", None)
            train_y = kwargs.pop("train_y")
            test_x = kwargs.pop("test_x")
            test_image = kwargs.pop("test_image", None)
    ...
    def _forward(  # noqa: PLR0912, C901
        ...
        *,
        ...
        if x["main"] is not None:
            seq_len, batch_size, _ = x["main"].shape
        else:
            batch_size, seq_len, _, _ = image.shape
        ...
        for k in y:
            ...
            if y[k].shape[1] < x_shape:
                assert (
                    y[k].shape[1] == single_eval_pos_
                    or y[k].shape[1] == x_shape
                )
        ...
        if x['main'] is None:
            ...
            embedded_x = self.token_append(embedded_x, image)
        
        del x, y, image
        
        embedded_x, embedded_y = self.add_embeddings( # add positional embeddings
            embedded_x,
            embedded_y,
            data_dags=data_dags,
            num_features=embedded_x.shape[-2],
            seq_len=seq_len,
            cache_embeddings=(
